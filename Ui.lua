...
local function UpdateSize1()
	...
	if OffsetY == 0 then
		ScrollTab.CanvasSize = UDim2.new(0, 580, 0, 450)
	else
...
DropPageLayout.Parent = DropdownFolder
	...
	function Tabs:CreateTab(TabConfig)
		...
		function Sections:AddSection(Title)
			...
			function Items:AddText(TextConfig)
				local TextConfig = TextConfig or {}
				TextConfig.Title = TextConfig.Title or "Title"
				...
				Text.LayoutOrder = CountItem
				Text.Size = UDim2.new(1, 0, 0, 29)
				Text.Name = "Text"
				Text.Parent = SectionAdd

				UICorner14.CornerRadius = UDim.new(0, 4)
				UICorner14.Parent = Text

				TextTitle.Font = Enum.Font.GothamBold
				TextTitle.Text = TextConfig.Title
				TextTitle.TextColor3 = Color3.fromRGB(230.77499270439148, 230.77499270439148, 230.77499270439148)
				TextTitle.TextSize = 14
				TextTitle.TextXAlignment = Enum.TextXAlignment.Left
				TextTitle.TextYAlignment = Enum.TextYAlignment.Top
			...
			function Items:AddDropdown(DropdownConfig)
				...
				UICornerSearch.CornerRadius = UDim.new(0, 5)
				...
				function DropdownFunc:Set(Value)
    				if DropdownConfig.Multi then
        				...
        				DropdownFunc.Value = type(Value) == "string" and Value or (Value[1] or "")
    				end

    				for _, Drop in ScrollSelect:GetChildren() do
        				if Drop:IsA("Frame") and Drop.Name == "Option" then
            				local isSelected = DropdownConfig.Multi 
                				and table.find(DropdownFunc.Value, Drop.OptionText.Text) 
    				...
    				if DropdownConfig.Multi then
        				...
        				OptionSelecting.Text = DropdownFunc.Value == "" and "Select Option" or DropdownFunc.Value
    				end
				...
				SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
					...
					for _, child in pairs(ScrollSelect:GetChildren()) do
						if child:IsA("Frame") and child.Name == "Option" then
							local optionName = string.lower(child.OptionText.Text)
							child.Visible = text == "" or optionName:find(text) == 1
						end
					end
				end)
